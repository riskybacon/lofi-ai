...
#if 0
template <typename T> Matrix<T> max(const Matrix<T> &mat, const size_t axis) {
    shape_type shape = mat.shape();
    shape[axis] = 1;
    Matrix<T> out(shape);
    max(mat.ctx_, axis);
    return out;
...
template <typename T> Matrix<T> sum(const Matrix<T> &mat, const size_t axis) {
    shape_type shape = mat.shape();
    shape[axis] = 1;
    Matrix<T> out(shape);
    sum(mat.ctx_, axis);
...
template <typename T> Matrix<T> pow(const Matrix<T> &mat, const T y) {
    Matrix<T> out(mat.shape());
    pow(out.ctx_, mat.ctx_, y);
    return out;
}

template <typename T> Matrix<T> exp(const Matrix<T> &mat) {
    Matrix<T> out(mat.shape());
    exp(out.ctx_, mat.ctx_);
    return out;
}

template <typename T> Matrix<T> tanh(const Matrix<T> &mat) {
    Matrix<T> out(mat.shape());
    tanh(out.ctx_, mat.ctx_);
    return out;
}

template <typename T> Matrix<T> log(const Matrix<T> &mat) {
    Matrix<T> out(mat.shape());
    log(out.ctx_, mat.ctx_);
    return out;
}
#endif
